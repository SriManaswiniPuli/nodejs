...
app.post("/movies", express.json(),function (request, response) {
    ...
    //const { v4: uuidv4 } = require("uuid");
    movies.push(data)
    response.send({msg:"movie added successfully",data:data})
  });
//express.json is middleware function here
...
app.put("/movies/:id", express.json(),function (request, response) {
    ...
    if(movieindex>0){
      ...
      response.send(movies[movieindex])
      
    }
    else{
      response.status(404).send({msg:"Movie not found"})
    }
    console.log(id,data,movieindex)
